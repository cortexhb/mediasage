...
from backend.recommender.matching import AlbumMatcher, pitch_matcher, selection_matcher
...
class TestExactMatch:
    ...
    def test_nothing_close_returns_none(self):
        assert selection_matcher().find(
            AlbumRef(artist="Nirvana", album="Nevermind"), entries(("Miles Davis", "Kind of Blue"))
        ) is None

    def test_empty_pool_returns_none(self):
        assert selection_matcher().find(AlbumRef(artist="a", album="b"), {}) is None
...
class TestFuzzyMatch:
    ...
    def test_best_scoring_candidate_wins(self):
        ...
        found = selection_matcher().find(AlbumRef(artist="Radiohead", album="The Bendz"), pool)
...
class TestConfiguredFloors:
    ...
    def test_a_lower_floor_admits_what_a_higher_one_rejects(self, monkeypatch, llm_config):
        ...
        assert selection_matcher().find(wanted, pool) is None
        ...
        assert selection_matcher().find(wanted, pool) is not None

    def test_each_matcher_reads_its_own_floors(self, llm_config):
        assert selection_matcher().artist_min == llm_config.matching.album_artist_min
        assert pitch_matcher().artist_min == llm_config.matching.pitch_artist_min
        assert pitch_matcher().album_min == llm_config.matching.pitch_album_min

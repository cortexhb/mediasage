...
from backend.recommender.models import AlbumRef, AnswerSet, ClarifyingQuestion, RecommendSession
...
class TestLifecycle:
    ...
    def test_unknown_id_returns_none(self, sessions):
        assert sessions.get("rec_doesnotexist") is None
    ...
    def test_deleting_twice_is_not_an_error(self, sessions):
        sessions.delete("rec_never_existed")
    ...
    def test_adopt_carries_live_flows_over(self, sessions):
        ...
        assert rebuilt.get(session_id).prompt == "migrated"


class TestEdits:
    def test_editing_an_unknown_session_is_a_no_op(self, sessions):
        sessions.set_answers("rec_gone", AnswerSet(answers=["calm"]))
    ...
    def test_start_round_keeps_candidates_when_not_given(self, sessions):
        """A "Show me another" round reuses the pool the first round built."""
        session_id = sessions.create(RecommendSession())
        sessions.start_round(session_id, "library", {}, "any", album_candidates=[])
    ...
    def test_spend_of_an_unknown_session_is_zero(self, sessions):
        ...
        assert sessions.spend("rec_gone") == (0, 0.0)

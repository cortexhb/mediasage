...
class TestPlaylistGeneration:
    ...
    def test_generate_validates_tracks_against_library(self, mocker, mock_plex_tracks):
        ...
        with patch("backend.generator.client_store") as mock_store:
            ...
            with patch("backend.generator.plex_store") as mock_plex:
                mock_plex_client = MagicMock()
                mock_plex_client.filtered.return_value = mock_plex_tracks[:5]
                mock_plex.get.return_value = mock_plex_client

                with (
                    patch("backend.generator.library.has_tracks", return_value=False),
                    patch("backend.generator.results_store.save", return_value="abc123"),
                ):
    ...
    def test_generate_handles_empty_filter_results(self, mocker):
        ...
        with patch("backend.generator.client_store") as mock_store:
            ...
            with patch("backend.generator.plex_store") as mock_plex:
                mock_plex_client = MagicMock()
                mock_plex_client.filtered.return_value = []
                mock_plex.get.return_value = mock_plex_client

                with patch("backend.generator.library.has_tracks", return_value=False):
                    events = _parse_sse_events(generate_playlist_stream(
...
class TestNarrativeGeneration:
    ...
    def test_generate_narrative_passes_through_long_narrative(self, mocker):
        ...
        _title, narrative = generate_narrative(track_selections, mock_client)

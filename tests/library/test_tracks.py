...
from backend.library import tracks
...
class TestFiltering:
    ...
    def test_live_versions_are_excluded_by_default(self, sample_library):
        assert tracks.count(TrackFilter()) == 3

    def test_live_versions_can_be_kept(self, sample_library):
        assert tracks.count(TrackFilter(exclude_live=False)) == 4

    def test_a_genre_filter_matches_case_insensitively(self, sample_library):
        assert tracks.count(TrackFilter(genres=["ROCK"], exclude_live=False)) == 3

    def test_several_genres_match_any_of_them(self, sample_library):
        assert tracks.count(TrackFilter(genres=["Rock", "Jazz"], exclude_live=False)) == 4

    def test_a_decade_filter_bounds_the_year(self, sample_library):
        assert tracks.count(TrackFilter(decades=["1990s"], exclude_live=False)) == 3

    def test_several_decades_match_any_of_them(self, sample_library):
        assert tracks.count(TrackFilter(decades=["1990s", "2000s"], exclude_live=False)) == 4

    def test_a_rating_filter_is_a_minimum(self, sample_library):
        assert tracks.count(TrackFilter(min_rating=5, exclude_live=False)) == 1

    def test_filters_combine(self, sample_library):
        assert tracks.count(TrackFilter(genres=["Rock"], decades=["2000s"])) == 0

    def test_an_unknown_genre_matches_nothing(self, sample_library):
        assert tracks.count(TrackFilter(genres=["Polka"])) == 0

    def test_filtered_returns_the_same_tracks_it_counts(self, sample_library):
        track_filter = TrackFilter(genres=["Rock"])
        assert len(tracks.filtered(track_filter)) == tracks.count(track_filter)
...
class TestSampling:
    ...
    def test_a_limit_caps_the_result(self, sample_library):
        assert len(tracks.filtered(TrackFilter(exclude_live=False), limit=2)) == 2

    def test_a_limit_applies_with_a_genre_filter(self, sample_library):
        assert len(tracks.filtered(TrackFilter(genres=["Rock"], exclude_live=False), limit=1)) == 1

    def test_no_limit_returns_everything(self, sample_library):
        assert len(tracks.filtered(TrackFilter(exclude_live=False), limit=0)) == 4

    def test_a_limit_beyond_the_library_is_harmless(self, sample_library):
        assert len(tracks.filtered(TrackFilter(exclude_live=False), limit=99)) == 4
...
class TestStats:
    ...
    def test_genres_are_counted_from_the_index(self, sample_library):
        stats = tracks.genre_decade_stats()
    ...
    def test_genres_are_sorted_by_name(self, sample_library):
        names = [genre.name for genre in tracks.genre_decade_stats().genres]
    ...
    def test_tracks_without_a_year_are_left_out(self, seed_tracks, year):
        ...
        assert tracks.genre_decade_stats().decades == []

...
from backend.llm import JSONParseError, ModelReply


def parsed(content: str):
    """The JSON recovered from one raw reply."""
    return ModelReply(content=content).parsed()
...
class TestSmartQuotes:
    ...
    def test_curly_single_quotes_are_normalised(self):
        ...
        result = parsed('[{"title": "Don\u2019t Stop"}]')
...
class TestExtraProse:
    ...
    def test_handles_nested_structures(self):
        ...
        assert parsed(content) == {"a": {"b": [1, 2, {"c": 3}]}}
...
class TestExtractJsonBounds:
    ...
    def test_ignores_brackets_inside_strings(self):
        ...
        assert ModelReply.bounds(content) == content
    ...
    def test_returns_none_without_a_structure(self):
        ...
        assert ModelReply.bounds("no json here") is None

    def test_returns_none_when_unbalanced(self):
        """An unclosed structure is not a result."""
        assert ModelReply.bounds('[{"a": 1}') is None
...
class TestRepair:
    ...
    def test_repairs_unescaped_quotes_in_a_value(self):
        ...
        result = parsed('[{"title": "The "Best" Song"}]')
...
class TestFailures:
    ...
    def test_error_carries_a_preview(self):
        ...
        with pytest.raises(JSONParseError, match="Response preview"):
            parsed("this is not json at all, not even close")

...
class TestPlaylist:
    def test_caps_what_is_sent(self):
        estimate = estimates.playlist(request(max_tracks_to_ai=100), 5000, PRICED)
    ...
    def test_no_cap_sends_everything(self):
        ...
        estimate = estimates.playlist(request(max_tracks_to_ai=0), 5000, PRICED)
        assert estimate.tracks_to_send == 5000

    def test_an_empty_library_sends_nothing(self):
        estimate = estimates.playlist(request(), 0, PRICED)
    ...
    def test_input_grows_with_the_tracks_sent(self):
        small = estimates.playlist(request(), 100, PRICED)
        large = estimates.playlist(request(), 200, PRICED)
    ...
    def test_output_grows_with_the_playlist_length(self):
        short = estimates.playlist(request(track_count=10), 100, PRICED)
        long = estimates.playlist(request(track_count=50), 100, PRICED)
    ...
    def test_an_unpriced_provider_reports_no_cost(self):
        ...
        assert estimates.playlist(request(), 500, mediasage_config()).estimated_cost == 0.0

    def test_a_priced_provider_reports_one(self):
        assert estimates.playlist(request(), 500, PRICED).estimated_cost > 0


class TestAlbums:
    def test_caps_what_is_sent(self):
        assert estimates.albums(5000, 2500, PRICED).albums_to_send == 2500

    def test_no_cap_sends_everything(self):
        assert estimates.albums(5000, 0, PRICED).albums_to_send == 5000

    def test_an_empty_library_sends_nothing(self):
        estimate = estimates.albums(0, 2500, PRICED)
    ...
    def test_input_grows_with_the_configured_album_size(self):
        estimate = estimates.albums(100, 0, PRICED)
        expected = (
    ...
    def test_an_unpriced_provider_reports_no_cost(self):
        assert estimates.albums(100, 0, mediasage_config()).estimated_cost == 0.0

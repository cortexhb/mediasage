...
from tests.api.conftest import connected_plex, mediasage_config
...
def analysable():
    ...
    plex = connected_plex()
    plex.track_by_key.return_value = TRACK
    plex.count.return_value = 500

    plex_store = MagicMock()
    plex_store.get.return_value = plex
    llm_store = MagicMock()
    llm_store.get.return_value = MagicMock()

    with (
        patch("backend.api.guards.plex_store", plex_store),
        patch("backend.api.guards.client_store", llm_store),
...
class TestAnalyzePrompt:
    def test_returns_the_analysis(self, client, analysable):
        ...
        with patch("backend.api.routes.analyze.analyze_prompt", return_value=answer):
    ...
    def test_an_unusable_reply_is_422(self, client, analysable):
        ...
        with patch(
            "backend.api.routes.analyze.analyze_prompt",
...
class TestAnalyzeTrack:
    def test_returns_the_dimensions(self, client, analysable):
        ...
        with patch("backend.api.routes.analyze.analyze_track", return_value=answer):
            response = client.post("/api/analyze/track", json={"rating_key": "1"})

        assert response.status_code == 200

    def test_a_missing_track_is_404(self, client, analysable):
        analysable.track_by_key.return_value = None
...
class TestFilterPreview:
    ...
    def test_counts_from_the_cache_when_there_is_one(self, client, analysable, temp_db):
        with (
            patch("backend.api.routes.analyze.library.has_tracks", return_value=True),
            patch("backend.library.tracks.count", return_value=1234) as counted,
        ):
            data = self.preview(client).json()

        counted.assert_called_once()
        analysable.count.assert_not_called()
        assert data["matching_tracks"] == 1234

    def test_falls_back_to_plex_without_a_cache(self, client, analysable, temp_db):
        with patch("backend.api.routes.analyze.library.has_tracks", return_value=False):
            data = self.preview(client).json()

        analysable.count.assert_called_once()
        assert data["matching_tracks"] == 500

    def test_a_plex_query_failure_is_502(self, client, analysable, temp_db):
        analysable.count.side_effect = PlexQueryError("timeout")
    ...
    def test_caps_what_would_be_sent(self, client, analysable, temp_db):
        with (
            patch("backend.api.routes.analyze.library.has_tracks", return_value=True),
            patch("backend.library.tracks.count", return_value=5000),
        ):

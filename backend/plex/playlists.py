...
Every write resolves rating keys first and acts on what resolved; a key Plex
has since dropped costs one track, not the operation. Entry points: `create`,
`listing`, `update`.
...
def create(
    connection: PlexConnection, name: str, rating_keys: list[str], description: str = ""
) -> PlaylistResult:
    ...
    try:
        ...
        playlist = connection.server.createPlaylist(name, items=fetched.items)
        _set_description(playlist, description)

        return PlaylistResult(
            success=True,
            playlist_id=str(playlist.ratingKey),
            playlist_url=connection.playlist_url(playlist.ratingKey),
...
def listing(connection: PlexConnection) -> list[PlexPlaylistInfo]:
    ...
    try:
        playlists = connection.server.playlists(playlistType="audio")
    except Exception:
...
def update(
    ...
    try:
        if playlist_id == SCRATCH_SENTINEL:
            with _scratch_lock:
                scratch = _find_scratch(connection)
                if scratch is None:
                    return _create_scratch(connection, rating_keys, description)
                playlist_id = str(scratch.ratingKey)

        playlist = connection.server.fetchItem(int(playlist_id))
        if mode == "replace":
            result = _replace(connection, playlist, rating_keys)
        else:
            result = _append(connection, playlist, rating_keys)

        if result.success:
            _set_description(playlist, description)
            result.playlist_url = connection.playlist_url(playlist.ratingKey)
        return result
    except Exception as error:
...
def _replace(connection: PlexConnection, playlist: Any, rating_keys: list[str]) -> PlaylistUpdateResult:
...
def _append(connection: PlexConnection, playlist: Any, rating_keys: list[str]) -> PlaylistUpdateResult:
    ...
    fetched = connection.fetch_items(wanted)
    if fetched:
        playlist.addItems(fetched.items)
...
def _find_scratch(connection: PlexConnection) -> Any | None:
    """The scratch playlist, or None when it has not been created yet."""
    try:
        for playlist in connection.server.playlists(playlistType="audio"):
...
def _create_scratch(
    connection: PlexConnection, rating_keys: list[str], description: str
) -> PlaylistUpdateResult:
    """Create the scratch playlist around its first set of tracks."""
    result = create(connection, SCRATCH_TITLE, rating_keys, description)
    return PlaylistUpdateResult(
...
def _set_description(playlist: Any, description: str) -> None:

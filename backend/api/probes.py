...
A probe is built against a candidate configuration: it asks a provider which
models it serves and reports what happened without raising. The settings page
runs one before writing, so a wrong credential is a form error rather than
something the user discovers on their next generation.

Plex is not probed here. Its address and token come from a browser sign-in,
which proves the server answers as part of choosing it -- see
`backend/api/routes/plex/link.py`.
...
from backend.config import ConfigUpdate, LLMSection, MediasageConfig

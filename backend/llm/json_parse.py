...
commas. `ModelReply.parsed` strips those in order and falls back to
`json_repair`.
...
class ModelReply(BaseModel):
    """One model's raw text, and the JSON that can be recovered from it."""
    ...
    @staticmethod
    def bounds(content: str) -> str | None:
        """The first complete JSON array or object embedded in `content`.

        Tracks bracket depth while skipping over string literals, so a bracket
        inside a title does not close the structure early. None when no
    ...
    def unfenced(content: str) -> str:
        """The contents of a fenced code block, or `content` unchanged."""
        match = JSON_FENCE.search(content) or ANY_FENCE.search(content)
        return match.group(1).strip() if match else content

    def parsed(self) -> Any:
        """The JSON this reply carries, however it was wrapped.
        ...
        content = self.unfenced(content).translate(SMART_QUOTES)

        try:
            return json.loads(content)
        except json.JSONDecodeError as original:
            extracted = self.bounds(content)

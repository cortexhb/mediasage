...
Four calls live here: the gap analysis that decides what to ask, the filter
suggestion that narrows the library, the question generation itself, and the
selection that picks the albums -- from the library, or from everything else.
...
from backend.recommender import dimensions, prompts
from backend.recommender.calls import MeteredClient, as_dict, as_list
from backend.recommender.matching import selection_matcher
...
def settings() -> RecommendConfig:
    """The configured shape of one round, read at call time."""
    return config_store.get().recommend


def gap_analysis(call: MeteredClient, prompt: str) -> list[str]:
    """The dimensions worth asking the user about.

    Returns:
        Exactly `dimensions.question_count()` valid dimension ids
    """
    raw = call.analyze(
        prompts.GAP_ANALYSIS_SYSTEM,
        prompts.gap_analysis(prompt, dimensions.catalogue()),
        "gap_analysis",
    )
    chosen = [item for item in as_list(raw) if isinstance(item, str)]
    return dimensions.fill(chosen)


def suggest_filters(
    call: MeteredClient, prompt: str, genres: list[str], decades: list[str]
) -> FilterSuggestion:
    """Which of the available genres and decades the prompt implies.

    Anything the model names that is not on offer is discarded; naming nothing
    usable means every option stays selected, which is the safe default.
    """
    raw = as_dict(
        call.generate(
    ...
    chosen_genres = [name for name in as_list(raw.get("genres")) if name in available_genres]
    chosen_decades = [name for name in as_list(raw.get("decades")) if name in available_decades]
...
def generate_questions(
    call: MeteredClient, prompt: str, dimension_ids: list[str]
) -> list[ClarifyingQuestion]:
    """Write one question per dimension, in the user's own terms."""
    lines = []
    for dimension_id in dimension_ids:
        dimension = dimensions.by_id(dimension_id)
    ...
    questions = []
    for item in as_list(raw)[: dimensions.question_count()]:
        fields = as_dict(item)
        questions.append(ClarifyingQuestion(
            question_text=str(fields.get("question_text", "") or ""),
            options=[str(option) for option in as_list(fields.get("options"))[:4]],
...
def select_albums(
    ...
    """
    round_shape = settings()
    pool = _remaining(candidates, already_shown)
    if len(pool) <= round_shape.pick_count:
        # Nothing to choose between: an LLM call would just echo the list back.
        return [_ranked(candidate, index) for index, candidate in enumerate(pool)]

    lines = [
        _album_line(candidate, familiarity_pref, familiarity, round_shape.genres_per_line)
    ...
    matcher = selection_matcher()
    picked = []
    for item in as_list(raw)[: round_shape.pick_count]:
        fields = as_dict(item)
        wanted = AlbumRef(
        ...
        if candidate is None:
            ...
            continue
        picked.append(AlbumRecommendation.of_candidate(candidate, _rank_of(fields)))

    return _with_a_primary(picked)
...
def select_discovery_albums(
    ...
    More are requested than shown: the exclusion list in the prompt is capped,
    so some of what comes back is already in the library and is filtered here.

    Args:
        max_exclusion_albums: Owned albums to list; the configured cap by default
    """
    round_shape = settings()
    excluded = (
        round_shape.max_exclusion_albums if max_exclusion_albums is None else max_exclusion_albums
    )
    raw = call.analyze(
    ...
    for item in as_list(raw)[: round_shape.discovery_request]:
...
def _remaining(
    candidates: list[AlbumCandidate], already_shown: list[AlbumRef] | None
) -> list[AlbumCandidate]:

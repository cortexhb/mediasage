...
Two calls: one reads Wikipedia, reviews and MusicBrainz metadata into labelled
fields, and one asks whether a discovery pick really matches what was asked
for. Entry points: `extract`, `matches_request`.
...
def extract(call: MeteredClient, ref: AlbumRef, research: ResearchData) -> ExtractedFacts:
    """Read the research for one album into labelled facts.

    `track_listing` is copied straight from MusicBrainz rather than extracted:
    it is the one field the fact-checker treats as authoritative.
    """
    raw = as_dict(
        call.generate(
            prompts.FACTS_SYSTEM,
            prompts.facts(ref, _sources(research)),
            "fact_extraction",
        )
    )
    ...
    return ExtractedFacts(
        ...
        personnel=[str(name) for name in as_list(raw.get("personnel"))],
...
def matches_request(
    call: MeteredClient, rec: AlbumRecommendation, research: ResearchData, prompt: str
) -> bool:
    """Whether a discovery pick genuinely fits what the user asked for.

    A model recommending from its own knowledge can name an album that exists
    but is nothing like the request. An unreadable answer counts as a failure:
    the user is told the pick could not be verified rather than shown it as if
    it had been.
    """
    raw = as_dict(
        call.generate(
            prompts.DISCOVERY_VALIDATION_SYSTEM,
            prompts.discovery_validation(prompt, _research_summary(rec, research)),

...
`parent_rating_key`. `album_cache` is the instance the application reads
through; its entry points are `candidates` and `familiarity`.
"""
...
class AlbumCache(BaseModel):
    """Albums as the cache can answer for them, grouped from their tracks."""

    def candidates(self, track_filter: TrackFilter) -> list[AlbumCandidate]:
        """Albums whose tracks survive a filter, assembled from those tracks.

        Genres are unioned across an album's tracks, matched case-insensitively
        so "Rock" and "rock" collapse; the first spelling Plex reported wins.

        Args:
            track_filter: Decade and live-version predicates; genres select
                which albums qualify rather than which tracks are kept

        Returns:
            One candidate per album, in no particular order
        """
        clauses = [Track.has_album_key(), *track_filter.clauses(include_genres=False)]
        ...
        with db.session() as session:
            for rating_key, artist, album, year, genres, parent_key in session.execute(statement):
    ...
    def familiarity(
        self, parent_rating_keys: list[str] | None = None
    ) -> dict[str, AlbumFamiliarity]:
        ...
        with db.session() as session:
            return {
                ...
                for key, total, average, last_viewed in session.execute(statement)
            }
